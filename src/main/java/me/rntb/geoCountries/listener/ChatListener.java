...
public class ChatListener implements Listener {
    ...
    private void doPrefixLogic(AsyncChatEvent event, PlayerProfile playerProfile) {
        ...
        if (!ConfigState.countryPrefixEnabled || playerCountry == null || playerCountry.settings.get("countryprefixenabled").equals("false") || playerCountry.settings.get("countryprefix").equals("null"))
            return;

        // build prefix and prepend
        ChatUtil.ChatColour chatColour = ChatUtil.ChatColour.WHITE;
        try {
            chatColour = ChatUtil.ChatColour.valueOf(playerCountry.settings.get("countryprefixcolour"));
        } catch (IllegalArgumentException ignored) { }
        Component prefix = Component.text((ConfigState.countryPrefixFormat + " ")
                                          .formatted(ChatUtil.getChatColourByEnum(chatColour),
                                                     playerCountry.settings.get("countryprefix")));
